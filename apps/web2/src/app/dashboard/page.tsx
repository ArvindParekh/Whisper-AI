...
export default function DashboardPage() {
   ...
   useEffect(() => {
      ...
      const registerTokenAndPoll = async () => {
         const res = await axios.post(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/register-token`,
            { token }
         );
         if (res.status === 200) {
            startPolling();
      ...
      registerTokenAndPoll();
   }, [token]);

   const startPolling = async () => {
      const interval = setInterval(async () => {
         const res = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/check-token?token=${token}`
         );
         const data = res.data;
            ...
            <main className='flex-1 p-6'>
               <div className='max-w-4xl mx-auto space-y-8'>
                  ...
                  <div>
                     ...
                     <p className='text-muted-foreground'>
                        Ready to continue coding with your voice?
                     </p>
                  </div>
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  ...
                  {status === "ready-to-join" && meetingId && (
                     <Button
                        onClick={() => {
